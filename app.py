...
import openai  # For the AI assistant endpoint
...
# Define a database model for Tutor Applications
...
# Create the database tables (run once, or use Flask-Migrate for production)
with app.app_context():
    db.create_all()

# Define folder paths for exam PDFs
EXAMS_FOLDER = os.path.join(os.getcwd(), "exams")
ANSWER_KEYS_FOLDER = os.path.join(os.getcwd(), "answer_keys")  # If using an answer_keys folder
...
def index():
    ...
    # Optionally, if you have answer key PDFs in another folder:
    answer_key_files = {}
...
@app.route('/tutors')
def tutors():
    # Optionally, query TutorApplication here to list stored applications
...
def become_tutor():
    if request.method == 'POST':
        ...
        # Validation: must fill required fields & at least phone or email
        if (not name or not location or not school or not hourly_rate or
            not experience or not classes_taught or (not phone and not email)):
            error = "Please fill in all required fields and provide at least a phone or email."
...
Email: {email}
        ...
        if email:
            try:
                ...
                <html>
                  <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                    <div style="max-width: 600px; margin: auto; border: 1px solid #e0e0e0; padding: 20px;">
                      ...
                      <p>Hello {name},</p>
                      <p>Thank you for applying to become a tutor with Tanzanian Exam PDFs. We have received your application, and due to a high volume of candidates, please allow 7–10 business days for an update regarding your application status.</p>
                ...
                # Log the error (optional); do not block the response.
...
# Enhanced AI Assistant Endpoint (example)
...
# REST API Endpoint to expose tutor data (for mobile apps, etc.)
